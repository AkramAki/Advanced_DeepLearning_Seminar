...
# http://pytorch.org/docs/stable/generated/torch.nn.utils.rnn.pad_sequence.html
...
def collate_fn_transformer(batch: list[dict]) -> tuple[list[list | torch.Tensor], torch.Tensor]:
    ...
    return [
        # (B, N, F)  -> (BxN, F) where B is the batch size, N is the number of hits, and F is the number of features (time, x, y)
        torch.cat(data_list),
        lengths,
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, data: tuple[torch.Tensor, list[int]]) -> torch.Tensor:
        ...
        # shape (B x N, D) -> (B, N, D), where every batch entry can have a variable length,
        ...
        # shape (B, N, D) -> (B x MAXLEN x D) now all batch entries have the same length
        ...
        mask = torch.zeros(batch_size, max_len, dtype=torch.bool).to(
            device=padded.device, dtype=torch.bool)
