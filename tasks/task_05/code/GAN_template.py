...
loader = DataLoader(datasets.MNIST(root="dataset/", transform=myTransforms,
...
class Generator(nn.Module):
    ...
    def __init__(self):
        ...
        self.gen = nn.Sequential(  # pyright: ignore
            # TODO define a suitable hidden dimension # pyright: ignore[reportArgumentType]
            nn.Linear(latent_dimension, ...)
            ...  # TODO define a suitable network architecture for the generator
            # It is helpful to use the tanh activation function to force the output into the [-1,1] range that our normalized images have.
            nn.Tanh(),
        )
...
for epoch in range(num_epochs):
    ...
    for batch_idx, (real, _) in enumerate(loader):
        ...
        print(
            ...
            f"\rEpoch [{epoch}/{num_epochs}] Batch {batch_idx}/{len(loader)}, Loss discriminator: {loss_discriminator:.4f}, loss generator: {loss_generator:.4f}",
            end="",
        )
